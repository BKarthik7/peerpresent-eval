...
export const RoomProvider: React.FC<{ children: ReactNode }> = ({
	...
	const [currentPresenter, setCurrentPresenter] = useState<Team | null>(
		() => {
			const savedPresenter = localStorage.getItem(
				STORAGE_KEYS.CURRENT_PRESENTER
			);
	...
	const [isPresentationActive, setIsPresentationActive] = useState<boolean>(
		() => {
			return (
				localStorage.getItem(STORAGE_KEYS.IS_PRESENTATION_ACTIVE) ===
				"true"
			);
		}
	);
	...
	const [isEvaluationActive, setIsEvaluationActive] = useState<boolean>(
		() => {
			return (
				localStorage.getItem(STORAGE_KEYS.IS_EVALUATION_ACTIVE) ===
				"true"
			);
		}
	);
	...
	useEffect(() => {
		...
		localStorage.setItem(
			STORAGE_KEYS.CURRENT_PRESENTER,
			currentPresenter ? JSON.stringify(currentPresenter) : ""
		);
		localStorage.setItem(
			STORAGE_KEYS.IS_PRESENTATION_ACTIVE,
			isPresentationActive.toString()
		);
		localStorage.setItem(
			STORAGE_KEYS.IS_EVALUATION_ACTIVE,
			isEvaluationActive.toString()
		);
	}, [
		isAdminLoggedIn,
		roomCode,
	...
	const startScreenShare = (stream: MediaStream) => {
		console.log(
			"Starting screen share with stream tracks:",
